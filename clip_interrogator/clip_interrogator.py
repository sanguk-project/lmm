...
# 데이터셋 경로 정의
dataset_path = pathlib.Path("/mnt/ssd/1/sanguk/lmm/datasets/test")
captions_path = pathlib.Path("/mnt/ssd/1/sanguk/lmm/datasets/captions")
...
with tqdm(total=len(image_files), desc="이미지 처리", unit="image") as pbar:
    for image_file in image_files:
        try:
            ...
            # 영어 캡션 생성
            caption_en = ci.interrogate(image)
            print(f"생성된 영어 캡션: {caption_en}")
            
            # 한국어로 번역
            try:
                caption_ko = translator.translate(caption_en, dest='ko').text
                print(f"번역된 한국어 캡션: {caption_ko}")
            except Exception as te:
                print(f"{image_file.name} 번역 오류: {str(te)}")
                caption_ko = None
            ...
            if caption_ko:
                item = {
                    ...
                    'caption': caption_ko  # 한국어 캡션 저장
                }
                with open(captions_file, 'a', encoding='utf-8') as f:
                    json.dump(item, f, ensure_ascii=False)
                    f.write('\n')
            else:
                print(f"{image_file.name}: 캡션 생성 또는 번역 실패")
